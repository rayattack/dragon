...
/* Dragon addition (not upstream): deterministic keypair from a caller-supplied
 * 32-byte seed. Byte-for-byte identical to crypto_sign_ed25519_keypair above
 * except the seed is provided instead of sampled via randombytes(), so a given
 * seed always yields the same (pk, sk). This is what lets Dragon use the RFC
 * 8032 32-byte secret-key seed as the private key and run known-answer tests. */
