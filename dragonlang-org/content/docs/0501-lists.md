...
```dragon
xs: list[int | str] = ["a", "b"]     # built as a box list from the start
```

But a *named* concrete list cannot be passed off as `list[int | str]`, and a
`list[int | str]` cannot be passed off as a concrete list - reading one layout at
...
# error: cannot assign 'list[str]' to variable of type 'list[int | str]' (the two
...
The same rule guards the boundary the compiler cannot see. An `Data` box can
...
`list[int | str]` and narrowing per element is the natural path:

```dragon
from json import decode
from json import Data

const doc: dict[str, Data] = decode[dict[str, Data]]('{"tags": ["a", "b"]}'.encode("utf-8"))
const tags: list[Data] = doc["tags"]    # parsed arrays are box lists - ok
for t in tags {
...
is exactly that shape. And note the rule pins the *typed view* `list[int | str]`,
not dynamism itself: a bare `Data` accepts any list and every operation on it
dispatches on the value's real layout at runtime. You can `len()` a `Data`
and iterate it directly - each element arrives as a `Data` to narrow - and
